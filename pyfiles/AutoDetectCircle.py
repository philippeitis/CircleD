...
def autoDetect(resized_img, accum_ratio, min_dist, p1, p2, minDiam, maxDiam, pixel_distance):
    ...
    minDist = max(1, int(min_dist * pixel_distance))
    minRadius = max(1, int(minDiam * pixel_distance / 2))
    maxRadius = max(1, int(maxDiam * pixel_distance / 2))
...
def tableData():
    ...
    if rad_list:
        ...
        col_list = [dict(Diam_um=items) for items in rad_list]
        row_list = ['rec' + str(row + 1) for row in range(len(rad_list))]
        dataForTable = dict(zip(row_list, col_list))

        try:
            if len(dataForTable) >= 1:
                temp_1 = dataForTable['rec1']['Diam_um']

            if len(dataForTable) >= 2:
                temp_2 = dataForTable['rec2']['Diam_um']

            if len(dataForTable) >= 3:
                temp_3 = dataForTable['rec3']['Diam_um']

            if len(dataForTable) >= 4:
                temp_4 = dataForTable['rec4']['Diam_um']

            if len(dataForTable) >= 5:
                temp_5 = dataForTable['rec5']['Diam_um']

            dataForTable.update({'rec1': {'Diam_um': str(temp_1), 'Col2': '# of Circles', 'Col3': str(len(rad_list))},
                                 'rec2': {'Diam_um': str(temp_2), 'Col2': 'Avg Diam (um)',
                                          'Col3': "%.1f" % np.average(rad_list)},
                                 'rec3': {'Diam_um': str(temp_3), 'Col2': 'D10 (um)',
                                          'Col3': str(rad_list[bottom_10percentile])},
                                 'rec4': {'Diam_um': str(temp_4), 'Col2': 'D50 (um)',
                                          'Col3': "%.1f" % np.median(rad_list)},
                                 'rec5': {'Diam_um': str(temp_5), 'Col2': 'D90 (um)',
                                          'Col3': str(rad_list[top_90percentile])}
                                 })
